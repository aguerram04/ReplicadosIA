...
export default function SubscribeButton() {
  ...
  const [keys, setKeys] = useState<{
    buyButtonIdSub: string;
    publishableKey: string;
  }>(() => ({
    buyButtonIdSub:
      process.env.NEXT_PUBLIC_STRIPE_BUY_BUTTON_ID_SUBSCRIPTION || "",
  ...
  const attrs = useMemo(() => {
    return {
      buyButtonId: keys.buyButtonIdSub,
  ...
  useEffect(() => {
    if (!keys.buyButtonIdSub || !keys.publishableKey) {
      fetch("/api/public/stripe")
        .then((r) => r.json())
        .then((j) => {
          if (j?.buyButtonIdSub || j?.publishableKey) {
            setKeys({
              buyButtonIdSub:
                j.buyButtonIdSub ||
                process.env.NEXT_PUBLIC_STRIPE_BUY_BUTTON_ID_SUBSCRIPTION ||
    ...
    <>
      ...
      {attrs.publishableKey && attrs.buyButtonId ? (
        ...
        <div className="px-4 py-2 rounded-md border border-white/20">
          Configura NEXT_PUBLIC_STRIPE_BUY_BUTTON_ID_SUBSCRIPTION y
